...
import numpy as np
from skimage import io as ioreadim
import cv2
import torch
import numpy as np
import SharedArray
...
from al3d_det.datasets.kitti.kitti_object_eval_python import kitti_common
import io
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.aug_with_img:
                if not 's3' in str(self.root_path):
                    ...
                    img_path = os.path.join(self.root_path, self.sampler_cfg.IMG_ROOT_PATH, (info['image_idx']+'.png'))
                    # raw_image = ioreadim.imread(img_path)
                    raw_image = cv2.imdecode(np.frombuffer(memoryview(self.client.get(img_path)), np.uint8), cv2.IMREAD_COLOR)
                
                # 修改 读取三通道的图像数据
                # 检查通道数
                if raw_image.shape[2] == 4:
                    raw_image = raw_image[:, :, :3]  # 只保留 RGB 通道
